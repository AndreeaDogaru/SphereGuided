...
class SphereRenderer(Renderer):
    def __init__(self, model, cfg, spheres, device=None, model_bg=None, **kwargs):
        super().__init__(model, cfg, device, model_bg, **kwargs)
    ...
    def unisurf(self, pixels, camera_mat, world_mat, 
        ...
        if self.render_empty:
            return super().unisurf(pixels, camera_mat, world_mat, scale_mat, add_noise=add_noise, it=it, eval_=eval_)
        # Get configs
        batch_size, n_points, _ = pixels.shape
        device = self._device
        rad = self.cfg['radius']
        ...
        valid_rays = mask_pred[0]  # Only finite elements

        # Merge rendering points
        p_fg = torch.zeros(batch_size * n_points, full_steps, 3, device=device)
        p_fg[~network_object_mask] =  p_noiter.view(-1, full_steps,3)
        p_fg[network_object_mask] =  p_iter.view(-1, full_steps,3)
        p_fg = p_fg[valid_rays].reshape(-1, 3)
        ray_vector_fg = ray_vector.unsqueeze(-2).repeat(1, 1, full_steps, 1)
        ray_vector_fg = -1*ray_vector_fg[:, valid_rays].reshape(-1, 3)


        rgb_fg_holder = p_fg.new_zeros(batch_size * n_points * full_steps, 3)
        logits_alpha_fg_holder = p_fg.new_zeros(batch_size * n_points * full_steps, 1)
        if p_fg.shape[0] > 0:
            # Run Network
            noise = not eval_
            rgb_fg, logits_alpha_fg = [], []
            for i in range(0, p_fg.shape[0], n_max_network_queries):
                ...
                rgb_fg.append(rgb_i)
                logits_alpha_fg.append(logits_alpha_i)
            valid_p_mask = valid_rays.view(1, n_points, 1).expand(batch_size, n_points, full_steps).contiguous()
            rgb_fg_holder[valid_p_mask.view(-1)] = torch.cat(rgb_fg, dim=0)
            logits_alpha_fg_holder[valid_p_mask.view(-1)] = torch.cat(logits_alpha_fg, dim=0)
        rgb_fg = rgb_fg_holder
        logits_alpha_fg = logits_alpha_fg_holder
    ...
    def ray_marching(self, ray0, ray_direction, model, c=None, 
        ...
        n_steps = torch.randint(n_steps[0], n_steps[1], (1,)).item()
        depth_intersect, _ = get_sphere_intersection(ray0[:,0], ray_direction, r=rad)
        far = depth_intersect[..., 1]
        near = torch.ones_like(far) * depth_range[0]
        
        d_proposal, _, ray_mask = sample_from_spheres(self.spheres, ray0, ray_direction, n_steps, near, far)
        d_proposal = fill_tensor(d_proposal.view(-1, n_steps), ray_mask.view(-1))  # Match the full dimension
        d_proposal = d_proposal.view(batch_size, n_pts, n_steps, 1)

        p_proposal = ray0.unsqueeze(2).repeat(1, 1, n_steps, 1) + \
            ray_direction.unsqueeze(2).repeat(1, 1, n_steps, 1) * d_proposal  # B x rays x steps x 3
        with torch.no_grad():
            parts = [(self.model(p_split, only_occupancy=True) - tau)
                for p_split in torch.split(p_proposal.reshape(batch_size, -1, 3), int(max_points / batch_size), dim=1)]
            val = torch.cat(parts, dim=1).view(batch_size, -1, n_steps)
        ...
        # Define mask where a valid depth value is found
        mask = mask_sign_change & mask_neg_to_pos & mask_0_not_occupied & ray_mask
        ...
        # for sanity
        d_pred_out = torch.ones(batch_size, n_pts).to(device)
        # Good rays - sample around depth
        d_pred_out[mask] = d_pred
        if self.render_empty:
            d_pred_out[mask == 0] = np.inf
            d_pred_out[mask_0_not_occupied == 0] = 0
        else:
            # No surface - linear sample
            d_pred_out[~mask] = 0
            # No sphere intersected - ignore
            d_pred_out[~ray_mask] = np.inf
        return d_pred_out
